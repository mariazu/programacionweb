<?php 
  session_start();
  include ('../layouts/header.php');
?>

 
    
     <!-- Inicio container -->
    <div class="container">
        <!-- fila 1 -->
        <div class="row">
            <div class="col-md-6">
              <img src="../images/img1.jpg" alt="..." class="img-rounded">
            </div>
            <div class="col-md-6">
                <div class="row">
                    <div class="col-md-6">
                        <img src="../images/img2.jpg" alt="..." class="img-rounded">
                    </div>
                </div>
                <div class="row">
                  <div class="col-md-12">
                    <ol class="breadcrumb">
                      <li><a href="#"><h5><span class="glyphicon glyphicon-file">Noticias</h5></a></li>
                      <li><a href="#"><h5><span class="glyphicon glyphicon-camera">Fotos</h5></a></li>
                      <li><a href="#"><h5><span class="glyphicon glyphicon-facetime-video">Videos</h5></a></li>
                      <li><a href="#"><h5><span class="glyphicon glyphicon-hdd">Juegos</h5></a></li>
                      <li><a href="#"><h5><span class="glyphicon glyphicon-screenshot">Actividad Social</h5></a></li>
                    </ol>
                </div>
                </div>
            </div>

        </div>
 <hr>
        <div class="row">
            <div class="col-md-12">
              <ul class="nav nav-pills">
  
                  <li class="dropdown">
                      <a class="dropdown-toggle" data-toggle="dropdown" href="#">
                      Partidos <span class="caret"></span>
                       </a>
                      <ul class="dropdown-menu" role="menu">
      
                       </ul>
                  </li>
                  <li class="dropdown">
                      <a class="dropdown-toggle" data-toggle="dropdown" href="#">
                      Equipos <span class="caret"></span>
                       </a>
                      <ul class="dropdown-menu" role="menu">
      
                       </ul>
                  </li>
                  <li class="dropdown">
                      <a class="dropdown-toggle" data-toggle="dropdown" href="#">
                      Jugadores <span class="caret"></span>
                       </a>
                      <ul class="dropdown-menu" role="menu">
      
                       </ul>
                  </li>
                  <li class="dropdown">
                      <a class="dropdown-toggle" data-toggle="dropdown" href="#">
                      Grupos <span class="caret"></span>
                       </a>
                      <ul class="dropdown-menu" role="menu">
      
                       </ul>
                  </li>
                  <li class="dropdown">
                      <a class="dropdown-toggle" data-toggle="dropdown" href="#">
                      Estad&iacute;sticas <span class="caret"></span>
                       </a>
                      <ul class="dropdown-menu" role="menu">
      
                       </ul>
                  </li>
                  <li class="dropdown">
                      <a class="dropdown-toggle" data-toggle="dropdown" href="#">
                      Premios <span class="caret"></span>
                       </a>
                      <ul class="dropdown-menu" role="menu">
      
                       </ul>
                  </li>
                  <li class="dropdown">
                      <a class="dropdown-toggle" data-toggle="dropdown" href="#">
                      Entradas <span class="caret"></span>
                       </a>
                      <ul class="dropdown-menu" role="menu">
      
                       </ul>
                  </li>
                  <li class="dropdown">
                      <a class="dropdown-toggle" data-toggle="dropdown" href="#">
                      Destino <span class="caret"></span>
                       </a>
                      <ul class="dropdown-menu" role="menu">
      
                       </ul>
                  </li>
                  <li class="dropdown">
                      <a class="dropdown-toggle" data-toggle="dropdown" href="#">
                      Organizaci&oacute;n <span class="caret"></span>
                       </a>
                      <ul class="dropdown-menu" role="menu">
      
                       </ul>
                  </li>
              </ul>
            </div>
        </div>

        <div class="row">
          <div class="col-md-12">
                <img src="../images/img3.jpg" alt="..." class="img-rounded">
          </div>
          
        </div>
        <div class="row">
          <div class="col-md-8">
                <h3>martes 01 julio</h3>
                <hr>
          </div>
          <div class="col-md-4">
              <div class="row">
                  <p><a class="ejemplo_4" class="btn btn-lg btn-success" href="suscripcion.html" role="button">
          Suscribirse</a></p>
              </div>
              <div class="row">
              </div>
          </div>
        </div>

        <div class="row">
          <div class="col-md-6">
              <div class="row">
                  <div class="col-md-4">
                      <div class="row">
                          <div class="col-md-4">
                            <h3>
                              <img width="40px" height="30px" src="../images/arg.png" alt="..." class="img-rounded">
                            </h3>
                          </div>
                          <div class="col-md-4">
                            
                            <div class="row">
                                  <h3>ARGENTINA</h3>
                            </div>
                            
                        </div>
                      </div>
                  </div>

                  <div class="col-md-4">
                      <div class="row">
                        <p class="text-center">FINAL DEL PARTIDO</p>
                      
                        <h4 class="text-primary" align="center">1-0</h4>
                      
                        <p class="text-center">Argentina gana en la pr&oacute;rroga</p>
                      </div>

                  </div>
                  <div class="col-md-4">
                      <div class="row">
                          
                          <div class="col-md-4">
                            
                            <div class="row">
                                  <h3>Suiza</h3>
                            </div>
                            
                        </div>
                        <div class="col-md-2">
                            <h3>
                              <img width="50px" height="30px" src="images/sui.png" alt="..." class="img-rounded">
                            </h3>
                          </div>
                      </div>
                  </div>
              </div>
              
         </div>
           <div class="col-md-6">
            <div class="row"> 
          
           
                  <div class="col-md-4">
                      <div class="row">
                          <div class="col-md-4">
                            <h3>
                              <img width="40px" height="30px" src="../images/bel.png" alt="..." class="img-rounded">
                            </h3>
                          </div>
                          <div class="col-md-4">
                            
                            <div class="row">
                                  <h3>B&Eacute;LGICA</h3>
                            </div>
                            
                        </div>
                      </div>
                  </div>

                  <div class="col-md-4">
                      <div class="row">
                        <p class="text-center">FINAL DEL PARTIDO</p>
                      
                        <h4 class="text-primary" align="center">2-0</h4>
                      
                        <p class="text-center">B&eacute;lgica gana en la pr&oacute;rroga</p>
                      </div>

                  </div>
                  <div class="col-md-4">
                      <div class="row">
                          
                          <div class="col-md-4">
                            
                            <div class="row">
                                  <h3>EEUU</h3>
                            </div>
                            
                        </div>
                        <div class="col-md-2">
                            <h3>
                              <img width="50px" height="30px" src="../images/usa_002.png" alt="..." class="img-rounded">
                            </h3>
                          </div>
                      </div>
                  </div>
              </div>
          </div>

        </div>

        <div class="row">
  
              <img width="88%" src="../images/cronica.jpg" alt="..." class="img-rounded">

        </div><br/>


        <div class="row">
            <div class="col-md-12">
              <!-- Fila 1 -->
               <div class="row">
                   <div class="col-md-4">
                       <div class="row">
                        <img  src="../images/2397535_large-lnd.jpg" alt="..." class="img-rounded">
                        </div>
                        <div class="row">
                          <h5>ESTADOS UNIDOS</h5>
                          <h4 class="text-primary">Howard asombra al mundo</h4>
                          <small>mi&eacute;rcoles 2 julio 2014</small>
                        </div> 
                   </div> 

                   <div class="col-md-4">
                        <div class="row">
                            <img  src="../images/2390864_large-lnd.jpg" alt="..." class="img-rounded">
                        </div>
                        <div class="row">
                          <h5>COLOMBIA</h5>
                          <h4 class="text-primary">Veinte años sin Escobar</h4>
                          <small>mi&eacute;rcoles 2 julio 2014</small>
                        </div>
                   </div> 

                   <div class="col-md-3">
                        <div class="row">             
                            <img width="80%" height="90%" src="../images/2388106_FULL-PRT.jpg" alt="..." class="img-rounded">
                       </div>
                   </div> 
               </div><br/>
              <!-- Fila 2 -->
               <div class="row">
                  <div class="col-md-4">
                      <div class="row">
                        <img width="80%" height="90%" src="../images/2389887_xbig-lnd.jpg" alt="..." class="img-rounded">
                        </div>
                        <div class="row">
                          <h5>PA&Iacute;SES BAJOS</h5>
                          <h4 class="text-primary">Robben:"Me estoy divirtiendo mucho"</h4>
                          <small>mi&eacute;rcoles 2 julio 2014</small>
                        </div> 
                   </div> 

                   <div class="col-md-4">
                      <div class="row">
                        <img width="80%" height="90%" src="../images/argentina.jpg" alt="..." class="img-rounded">
                        </div>
                        <div class="row">
                          <h5>BRAZIL 2014</h5>
                          <h4 class="text-primary">Partidos clásicos:Argentina-B&eacute;lgica en España 1982</h4>
                          <small>mi&eacute;rcoles 2 julio 2014</small>
                        </div>
                   </div> 

                   <div class="col-md-4">
                       <div class="row">             
                            <img width="80%" height="90%" src="../images/azul.jpg" alt="..." class="img-rounded">
                       </div>
                   </div>

               </div>

               <!-- Fila 3 -->
               <div class="row">
                <div align="right" class="col-md-12">
                    <ol class="breadcrumb">
                      <li><a href="#">Todas Las Noticias</a></li>
                      <li><a href="#">M&aacute;s Fotos></a></li>
                      <li><a href="#">M&aacute;s Videos</a></li>
                      <li><a href="#">Archivo De Live Blogs</a></li>
                    </ol>
                </div>
               </div>
 
            </div>
        </div><br/>
        <div class="row">
            <div class="col-md-12">

                <div class="row">
                  <h3>Disfruta tu experiencia Brasil 2014</h3>
                  <p>_______________________________________________________________________________________________________________________________________________________</p>
                </div>

                <div class="row">
                    <div class="col-md-4">
                        <img src="../images/Ticketing_es.png" alt="..." class="img-rounded">
                    </div>

                    <div class="col-md-4">
                        <img src="../images/Accomodation_es.png" alt="..." class="img-rounded">
                    </div>

                    <div class="col-md-4">
                        <img src="../images/Hospitality_es.png" alt="..." class="img-rounded">
                    </div>
                </div>
            </div>
        </div><br/><br/>

        <div class="row">
            <div class="col-md-12">

                <div class="row">
                  <h3>Torneos FIFA</h3>
                  <p>_______________________________________________________________________________________________________________________________________________________</p>
                </div>

                <div class="row">
                    <div class="col-md-4">
                        <img src="../images/noticias.jpg" alt="..." class="img-rounded">
                    </div>

                    <div class="col-md-4">
                        <img src="../images/partidos.jpg" alt="..." class="img-rounded">
                    </div>

                    <div class="col-md-4">
                        <img src="../images/partidos2.jpg" alt="..." class="img-rounded">
                    </div>
                </div>
            </div>
        </div>

        <div class="row">
            <div class="col-md-12">

                <div class="row">
                  <h3>Juegos</h3>
                  <p>_______________________________________________________________________________________________________________________________________________________</p>
                </div>

                <div align="right" class="row">
                  <ol class="breadcrumb">
                      <li><a href="#"><</a></li>
                      <li><a href="#">></a></li>
                    </ol>
                </div>

                <div class="row">
                    <div class="col-md-3">
                        <div class="row">
                            <h5>¡PRONOSTICADOR CASTROL!</h5>
                        </div>
                        <div class="row">
                            <img width="100%" height="100%" src="../images/2334697_large-lnd.jpg" alt="..." class="img-rounded">
                        </div>
                        <div align="center" class="row">
                            <ol class="breadcrumb">
                                <li><a href="#">¡Pronosticador Castrol!></a></li>
                            </ol>
                        </div>

                    </div>
                    <div class="col-md-3">
                        <div class="row">
                            <h4>F&Uacute;TBOL FANTASY 2014</h4>
                        </div>
                        <div class="row">
                            <img width="100%" height="100%" src="../images/2338253_large-lnd.jpg" alt="..." class="img-rounded">
                        </div>
                        <div align="center" class="row">
                            <ol class="breadcrumb">
                                <li><a href="#">f&uacute;tbol Fantasy 2014></a></li>
                            </ol>
                        </div>
                      </div>
                        <div class="col-md-3">
                        <div class="row">
                            <h4>TUS 11 TITULARES</h4>
                        </div>
                        <div class="row">
                            <img width="100%" height="100%" src="../images/2341589_large-lnd.jpg" alt="..." class="img-rounded">
                        </div>
                        <div align="center" class="row">
                            <ol class="breadcrumb">
                                <li><a href="#">Tus 11 Titulares></a></li>
                            </ol>
                        </div>
                      </div>
                        <div class="col-md-3">
                        <div class="row">
                            <h4>JUEGO DEL ONCE IDEAL</h4>
                        </div>
                        <div class="row">
                            <img width="100%" height="100%" src="../images/2341591_large-lnd.jpg" alt="..." class="img-rounded">
                        </div>
                        <div align="center" class="row">
                            <ol class="breadcrumb">
                                <li><a href="#">Juego Del Once Ideal></a></li>
                            </ol>
                        </div>

                    </div>

                </div>
                </div>
        </div><br/><br/>

        <div class="row">
            <div class="col-md-12">
               <div class="row">
                  <div class="col-md-8">
                      <h3>Destino</h3>
                      <p>_______________________________________________________________________________________________________________________________________________________</p>
                  </div>

                  <div class="col-md-2">
                      <h4>Presentado por</h4>
                  </div>
               

                    <div class="col-md-2">
                      <h2 class="text-danger">Emirates</2>
                    </div>
               </div>

               <div class="row">
                  <div align="center" class="col-md-6">
                      <img width="100%" src="../images/mapa.jpg" alt="..." class="img-rounded">
                  </div>

                  <div class="col-md-6">
                      <div class="row">
                        <div align="center" class="col-md-2">
                             <img width="70px" height="70px" src="../images/1037338_small.jpg" alt="..." class="img-rounded">
                        </div>

                        <div class="col-md-4">
                            <h4 class="text-primary">Belo Horizonte</h4>
                            <h5 class="text-primary">Estadio Mineirao</h5>
                        </div>

                        <div class="col-md-2">

                            <img width="70px" height="70px" src="../images/738308_small.jpg" alt="..." class="img-rounded">
                        </div>

                        <div class="col-md-4">
                          <h4 class="text-primary">Brasilia</h4>
                          <h5 class="text-primary">Estadio Nacional</h5>
                        </div>
                      </div><br/>

                      <div class="row">
                        <div align="center" class="col-md-2">
                             <img width="70px" height="70px" src="../images/1037321_small.jpg" alt="..." class="img-rounded">
                        </div>

                        <div class="col-md-4">
                            <h4 class="text-primary">Cuiab&aacute;</h4>
                            <h5 class="text-primary">Avena Pantanal</h5>
                        </div>

                        <div class="col-md-2">

                            <img width="70px" height="70px" src="../images/1038566_small.jpg" alt="..." class="img-rounded">
                        </div>

                        <div class="col-md-4">
                          <h4 class="text-primary">Curitiba</h4>
                          <h5 class="text-primary">Avena de Baixada</h5>
                        </div>
                      </div><br/>

                      <div class="row">
                        <div align="center" class="col-md-2">
                             <img width="70px" height="70px" src="../images/1037323_small.jpg" alt="..." class="img-rounded">
                        </div>

                        <div class="col-md-4">
                            <h4 class="text-primary">Fortaleza</h4>
                            <h5 class="text-primary">Estadio Castelao</h5>
                        </div>

                        <div class="col-md-2">

                            <img width="70px" height="70px" src="../images/1037325_small.jpg" alt="..." class="img-rounded">
                        </div>

                        <div class="col-md-4">
                          <h4 class="text-primary">Manaos</h4>
                          <h5 class="text-primary">Arena Amazonia</h5>
                        </div>
                      </div><br/>

                      <div class="row">
                        <div align="center" class="col-md-2">
                             <img width="70px" height="70px" src="../images/1037328_small.jpg" alt="..." class="img-rounded">
                        </div>

                        <div class="col-md-4">
                            <h4 class="text-primary">Natal</h4>
                            <h5 class="text-primary">Estadio das Dunas</h5>
                        </div>

                        <div class="col-md-2">

                            <img width="70px" height="70px" src="../images/1037329_small.jpg" alt="..." class="img-rounded">
                        </div>

                        <div class="col-md-4">
                          <h4 class="text-primary">Porto Alegre</h4>
                          <h5 class="text-primary">Estadio Beira-Rio</h5>
                        </div>
                      </div><br/>

                      <div class="row">
                        <div align="center" class="col-md-2">
                             <img width="70px" height="70px" src="../images/1037331_small.jpg" alt="..." class="img-rounded">
                        </div>

                        <div class="col-md-4">
                            <h4 class="text-primary">Recife</h4>
                            <h5 class="text-primary">Arena Pernambuco</h5>
                        </div>

                        <div class="col-md-2">

                            <img width="70px" height="70px" src="../images/1017789_small.jpg" alt="..." class="img-rounded">
                        </div>

                        <div class="col-md-4">
                          <h4 class="text-primary">Rio de Janeiro</h4>
                          <h5 class="text-primary">Estadio Maracan&aacute;</h5>
                        </div>
                      </div><br/>

                      <div class="row">
                        <div align="center" class="col-md-2">
                             <img width="70px" height="70px" src="../images/1017718_small.jpg" alt="..." class="img-rounded">
                        </div>

                        <div class="col-md-4">
                            <h4 class="text-primary">Salvador de Bah&iacute;a</h4>
                            <h5 class="text-primary">Arena Fonte Nova</h5>
                        </div>

                        <div class="col-md-2">

                            <img width="70px" height="70px" src="../images/1017629_small.jpg" alt="..." class="img-rounded">
                        </div>

                        <div class="col-md-4">
                          <h4 class="text-primary">Sao Paulo</h4>
                          <h5 class="text-primary">Arena de Sao Paulo</h5>
                        </div>
                      </div>
                  </div>
               </div>

            </div>
        </div>

        <div class="row">

                  <h3>Organizaci&oacute;n</h3>
                  <p>_______________________________________________________________________________________________________________________________________________________</p>
                </div>

                <div align="right" class="row">
                  <ol class="breadcrumb">
                      <li><a href="#"><</a></li>
                      <li><a href="#">></a></li>
                    </ol>
                </div>

                <div class="row">
                    <div class="col-md-4">
                        <div class="row">
                            <h4>FOOTBALL FOR HOPE</h4>
                        </div>
                        <div class="row">
                            <img src="../images/2387089_large-lnd.jpg" alt="..." class="img-rounded">
                        </div>
                        <div align="center" class="row">
                            <ol class="breadcrumb">
                                <li><a href="#">Football For Hope></a></li>
                            </ol>
                        </div>

                    </div>
                    <div class="col-md-4">
                        <div class="row">
                            <h4>TOUR DEL TROFEO</h4>
                        </div>
                        <div class="row">
                            <img src="../images/2172844_large-lnd.jpg" alt="..." class="img-rounded">
                        </div>
                        <div align="center" class="row">
                            <ol class="breadcrumb">
                                <li><a href="#">Tour Del Trofeo</a></li>
                            </ol>
                        </div>
                      </div>
                        <div class="col-md-4">
                        <div class="row">
                            <h4>LA VISI&Oacute;N DEL VALCKE</h4>
                        </div>
                        <div class="row">
                            <img src="../images/2300546_large-lnd.jpg" alt="..." class="img-rounded">
                        </div>
                        <div align="center" class="row">
                            <ol class="breadcrumb">
                                <li><a href="#">La Visi&oacute;n De Valcke></a></li>
                            </ol>
                        </div>
                      </div>
                </div><br/>

                <div class="row">
                  
                      <div class="col-md-6">
                          <img src="../images/committee-col6.png" alt="..." class="img-rounded">
                      </div>
                      <div class="col-md-6">
                        <img src="../images/volunteers-col6.png" alt="..." class="img-rounded">
                     </div>                   
                </div>

                <div class="row">
          
                  <h3>La FIFA</h3>
                  <p>_______________________________________________________________________________________________________________________________________________________</p>
               </div>
               <div class="row">
                    <div class="col-md-4">
                        <div class="row">
                            <h4>PRENSA</h4>
                        </div>
                        <div class="row">
                            <img src="../images/2388908_large-lnd.jpg" alt="..." class="img-rounded">
                        </div><br/>
                        <div class="row">
                            <p>La fase de grupos de la Copa Mundial de la FIFA<br/> Brasil 2014 ha alcanzado r&eacute;cords de<br/> telespectadores por todo el globo, con un<br/> desempeño espectacular en Estados Unidos y<br/> nuevos avances sensacionales en Europ, Asia y el<br/> resto de Am&eacute;rica.</p>
                        </div>
                        <div align="center" class="row">
                            <ol class="breadcrumb">
                                <li><a href="#">Noticias></a></li>
                                <li><a href="#">El Canal De Los Medios></a></li>
                            </ol>
                        </div>

                    </div>
                    <div class="col-md-4">
                        <div class="row">
                            <h4>THE FIFA WEEKLY</h4>
                        </div>
                        <div class="row">
                            <img width="85%"src="../images/revista.jpg" alt="..." class="img-rounded">
                        </div><br/>
                        <div class="row">
                            <p>An&aacute;lisis, reportajes, im&aacute;genes. La revista The FIFA<br/>Weekly aparece todos los viernes, tanto en papel<br/>como en formato electr&oacute;nico</p>
                        </div>
                        <div align="center" class="row">
                            <ol class="breadcrumb">
                                <li><a href="#">The FIFA Weekly></a></li>
                            </ol>
                        </div>
                      </div>
                        <div class="col-md-4">
                        <div class="row">
                            <h4>LA FIFA</h4>
                        </div>
                        <div class="row">
                            <img src="../images/2300217_large-lnd.jpg" alt="..." class="img-rounded">
                        </div><br/>
                        <div class="row">
                            <p>FIFA.com te cuenta la experiencia en los estadios<br/>de varios hinchas con discapacidad visual. El<br/>servicio de narraci&oacute;n audiodescriptiva de la Copa<br/>Mundial de la FIFA est&aacute; coordinado por Urece<br/>Esporte e Cultura para Cegos, una ONG que<br/>colabora con la FIFA y el COL.</p>
                        </div>
                        <div align="center" class="row">
                            <ol class="breadcrumb">
                                <li><a href="#">Todas Las Noticias></a></li>
                            </ol>
                        </div>
                      </div>
                </div><br/>

              <div class="row">
                  <div class="col-md-12">
                      <div class="row">
                          <h3>Todas las Copas Mundiales</h3>
                          <p>_______________________________________________________________________________________________________________________________________________________</p>
                      </div>

                      <div class="row">
                          <div align="center" class="col-md-3">    
                              <button type="button" class="btn btn-link">1930/1958</button>
                          </div>

                          <div align="center" class="col-md-2">
                             <button type="button" class="btn btn-link">1962/1982</button>
                          </div>

                          <div align="center" class="col-md-2">
                             <button type="button" class="btn btn-link">1986/2006</button>
                          </div>

                          <div align="center" class="col-md-2">
                             <button type="button" class="btn btn-link">2010/2022</button>
                          </div>

                          <div align="right" class="col-md-3">
                            <ol class="breadcrumb">
                                <li><a href="#"><</a></li>
                                <li><a href="#">></a></li>
                            </ol>
                          </div>

                      </div>

                      <div class="row">
                          <div align="center" class="col-md-2">
                              <div class="row">
                                <h4 class="text-primary">Corea/jap&oacute;n 2002</h4>

                              </div>
                              <div class="row">
                                <img src="../images/2002fwcoe_4cf_ps.png" alt="..." class="img-rounded">
                              </div>
                          </div>

                          <div align="center" class="col-md-2">
                              <div class="row">
                                <h4 class="text-primary">Alemania 2006</h4>
                              </div>
                              <div class="row">
                                <img src="../images/2006fwcoe_4cf_ps.png" alt="..." class="img-rounded">
                              </div>
                          </div>

                          <div align="center" class="col-md-2">
                              <div class="row">
                                <h4 class="text-primary">Sud&aacute;frica 2010</h4>
                              </div>
                              <div class="row">
                                <img src="../images/2010fwcoe_4cf_ps.png" alt="..." class="img-rounded">
                              </div>
                          </div>

                          <div align="center" class="col-md-2">
                             <div class="row">
                                <h4 class="text-primary">Brasil 2014</h4>
                              </div>
                              <div class="row">
                                <img src="../images/2014fwcoe_4cf_ps.png" alt="..." class="img-rounded">
                              </div> 
                          </div>

                          <div align="center" class="col-md-2">
                              <div class="row">
                                <h4 class="text-primary">Rusia 2018</h4>
                              </div>
                              <div class="row">
                                <img src="../images/17_2018.png" alt="..." class="img-rounded">
                              </div>
                          </div>

                          <div align="center" class="col-md-2">
                              <div class="row">
                                <h4 class="text-primary">Qatar 2022</h4>
                              </div>
                              <div class="row">
                                <img src="../images/17_2022.png" alt="..." class="img-rounded">
                              </div>
                          </div>
                      </div><br/>

                      <div class="row">
                          <div class="col-md-4">
                            <img src="../images/2250600_large-lnd.jpg" alt="..." class="img-rounded">
                          </div>

                          <div class="col-md-4">
                            <h4 class="text-primary">El COL de Rusiia 2018,<br/> entristecido por los hechos de</br> Volgogrado</h4>
                            <p>El COL de la Copa Mundial de la FIFA Rusia<br/>2013 quiere hacer llegar a los familiares y<br/>amigos de las v&iacute;ctimas de los tr&aacute;gicos eventos<br/>sucedidos en Volgogrado sus m&aacute;s sinceras<br/>condolencias.</p>
                          </div>

                          <div class="col-md-4">
                          </div>
                      </div>
                       
                       <div align="right" class="row">
                          <ol class="breadcrumb">
                                <li><a href="#">Rusia 2018></a></li>    
                            </ol>
                      </div> 
                  </div>
              </div>

              <div class="row">
                  <div class="col-md-12">
                      <div align="center" class="row">
                          <h5>Socios FIFA</h5>
                      </div>
                      <div align="center" class="row">
                       
                              <img src="../images/socios.jpg" alt="..." class="img-rounded">  
                      </div><br/>

                      <div align="center" class="row">
                          <h5>Patrocinadores de la Copa Mundial de la FIFA</h5>
                      </div>
                      <div align="center" class="row">
                       
                              <img src="../images/patrocinador.jpg" alt="..." class="img-rounded">  
                      </div><br/>

                      <div align="center" class="row">
                          <h5>Promotores nacionales</h5>
                      </div>
                      <div align="center" class="row">
                       
                              <img src="../images/promotores.jpg" alt="..." class="img-rounded">  
                      </div>

                  </div>
              </div><br/>

                  <div class="row">
                          <div align="center" class="col-md-3">    
                              <button type="button" class="btn btn-link">Partidos</button>
                          </div>

                          <div align="center" class="col-md-2">
                             <button type="button" class="btn btn-link">Equipos</button>
                          </div>

                          <div align="center" class="col-md-2">
                             <button type="button" class="btn btn-link">Entradas</button>
                          </div>

                          <div align="center" class="col-md-2">
                             <button type="button" class="btn btn-link">Destino</button>
                          </div>

                          <div align="center" class="col-md-3">
                            <button type="button" class="btn btn-link">Organizaci&oacute;n</button>
                          </div>

                      </div>


                <div class="row">
                  <div align="right" class="col-md-12">
                    <ol class="breadcrumb">
                      <li><a href="#"><h5>FIFA</h5></a></li>
                      <li><a href="#"><h5>Terminos del servicio</h5></a></li>
                      <li><a href="#"><h5>Pol&iacute;tica de privacidad</h5></a></li>
                      <li><a href="#"><h5>Desc&aacute;rgalo</h5></a></li>
                      <li><a href="#"><h5>Contacto con la FIFA</h5></a></li>
                      <li><a href="#"><h5>Versi&oacute;n Celular</h5></a></li>
                    </ol>
                </div>
                </div>


                </div>
        </div><br/><br/>
        



        
      <!-- Fin container -->
      </div>



<?php include ('../layouts/footer.php'); ?>
